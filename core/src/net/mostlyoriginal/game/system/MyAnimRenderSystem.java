...
public class MyAnimRenderSystem extends DeferredEntityProcessingSystem  {
	...
	public MyAnimRenderSystem(EntityProcessPrincipal principal) {
		super(Aspect.all(Pos.class, Anim.class, Render.class)
				.exclude(Invisible.class), principal);
	}
	...
	protected void process(final int e) {
		...
		if (anim.id != null)
			drawAnimation(anim, angle, origin, pos, anim.id, scale);
		if (anim.id2 != null)
			drawAnimation(anim, angle, origin, pos, anim.id2, scale);
	...
	private void drawAnimation(final Anim animation, final Angle angle,
			final Origin origin, final Pos position, String id, float scale) {

		// don't support backwards yet.
		if (animation.age < 0)
			return;

		final Animation<TextureRegion> gdxanim = (Animation<TextureRegion>) abstractAssetSystem
				.get(id);
		if (gdxanim == null)
			return;

		final TextureRegion frame = gdxanim.getKeyFrame(animation.age,
				gdxanim.getPlayMode() != Animation.PlayMode.NORMAL
						&& animation.loop);
		...
		if (animation.flippedX && angle.rotation == 0) {
			// mirror
			batch.draw(frame.getTexture(), roundToPixels(position.xy.x),
					roundToPixels(position.xy.y), ox, oy,
					frame.getRegionWidth() * scale,
					frame.getRegionHeight() * scale, 1f, 1f, angle.rotation,
					frame.getRegionX(), frame.getRegionY(),
					frame.getRegionWidth(), frame.getRegionHeight(), true,
					false);

		} else if (angle.rotation != 0) {
			batch.draw(frame, roundToPixels(position.xy.x),
					roundToPixels(position.xy.y), ox, oy,
					frame.getRegionWidth() * scale,
					frame.getRegionHeight() * scale, 1, 1, angle.rotation);
		} else {
			batch.draw(frame, roundToPixels(position.xy.x),
